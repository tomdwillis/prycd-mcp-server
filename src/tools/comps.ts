...
interface GeographyResponseItem {
  county: string;
  comps: Array<{
    acreage: string;
    comps: CompItem[];
...
interface GeographyResponse {
  status: number;
  data: GeographyResponseItem[];
}

interface LatLongResponse {
  status: number;
  data: LatLongResponseItem[];
}

// --- Formatters ---

function formatComp(comp: CompItem, distance?: number): string {
  const lines: string[] = [];
  ...
  if (distance !== undefined) lines.push(`  Distance: ${distance.toFixed(2)} mi`);
  if (comp.list_date) lines.push(`  Listed: ${comp.list_date}`);
  if (comp.sold_date) lines.push(`  Sold: ${comp.sold_date}`);
  if (comp.url) lines.push(`  Listing: ${comp.url}`);
  return lines.join("\n");
}

function formatGeographyResponse(data: GeographyResponseItem[]): string {
  const sections: string[] = [];
  let totalComps = 0;

  for (const countyData of data) {
    const countyLines: string[] = [`## ${countyData.county}`, ""];
...
function formatLatLongResponse(data: LatLongResponseItem[]): string {
  const lines: string[] = [`**${data.length} comps found**`, ""];
...
function buildCompOptions(options?: z.infer<typeof compOptionsSchema>): Record<string, unknown> | undefined {
  if (!options) return undefined;
  const opts: Record<string, unknown> = {};
  if (options.min_price !== undefined) opts.min_price = options.min_price;
  if (options.max_price !== undefined) opts.max_price = options.max_price;
  if (options.city) opts.city = options.city;
  if (options.zip_code) opts.zip_code = options.zip_code;
  if (options.remove_duplicates !== undefined) opts.remove_duplicates = options.remove_duplicates;
  if (options.excluded_sources?.length) opts.excluded_sources = options.excluded_sources;
  if (options.comp_type !== undefined) opts.comp_type = options.comp_type;
  if (options.comp_age !== undefined) opts.comp_age = options.comp_age;
  if (options.get_historical !== undefined) opts.get_historical = options.get_historical;
  if (options.only_valid_lat_long !== undefined) opts.only_valid_lat_long = options.only_valid_lat_long;
  return Object.keys(opts).length > 0 ? opts : undefined;
}

// --- Tool Registration ---

export function registerCompTools(server: McpServer): void {
  // Comps by County
  server.registerTool(
    "prycd_comps_by_county",
    {
...
Returns: Comps organized by county and acreage range, each with price, acreage, price per acre, status, source, listing URL, and dates.`,
    ...
    async (params) => {
      ...
      if (opts) body.comp_options = opts;

      const data = await apiRequest<GeographyResponse>("compsByCounty", { body });
...
Returns: Comps organized by county and acreage range.`,
    ...
    async (params) => {
      ...
      const data = await apiRequest<GeographyResponse>("compsByState", { body });
...
Returns: Comps sorted by distance from the center point, each with distance in miles.`,
    ...
    async (params) => {
      ...
      if (params.max_comps) body.max_comps = params.max_comps;
      const opts = buildCompOptions(params.comp_options);
      if (opts) body.comp_options = opts;

      const data = await apiRequest<LatLongResponse>("compsByLatLong", { body });
...
Returns: Status message from the Prycd API.`,
    ...
    async () => {
      const data = await apiRequest<{ status: number; message: string }>("healthcheck", {
        method: "GET",
      });
      return {
